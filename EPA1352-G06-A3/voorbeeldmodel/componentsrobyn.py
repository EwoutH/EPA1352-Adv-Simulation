...
class Bridge(Infra):
    ...
    def __init__(self, unique_id, model, length=0,
        ...
        if self.probabilities is not None:
            if self.condition == 'A':
                if self.random.random() < self.probabilities['A'] / 100:
                    self.broken = True
            elif self.condition == 'B':
                if self.random.random() < self.probabilities['B'] / 100:
                    self.broken = True
            elif self.condition == 'C':
                if self.random.random() < self.probabilities['C'] / 100:
                    self.broken = True
            else:
                if self.random.random() < self.probabilities['D'] / 100:
                    self.broken = True
        ...
        # print("Bridge " + str(self.unique_id) + ": " + str(self.condition) + " Delay:" + str(self.delay_time))
...
class Vehicle(Agent):
    ...
    def set_path(self):
        ...
        # if [self.source, self.sink] in self.path_ids_dict:
        #     self.path_ids = self.path_ids_dict[self.source, self.sink]
        # else:
